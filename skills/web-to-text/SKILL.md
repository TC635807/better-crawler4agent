...
- `warning` 非空表示**拿到了内容但可能不完整**（见下）
- `content_length` 是**截断前**的真实长度，`truncated` 表示返回的 `content` 被 `max_chars` 截过

### 异步渲染的站点会自动重试

有些站点的内容靠接口异步填充（检索结果页、SPA），接口会概率性失败，页面停在只有导航的骨架状态。工具会**自动重试最多 3 次**，并取历次中最完整的一次。

因此可能看到两种额外情况：

- `attempts` 里有多条 `browser` 记录 —— 说明重试过，最后一条是采用的结果
- **`ok: true` 但 `warning` 非空** —— 重试后仍只拿到少量内容。这时内容**可能就是不全的**，需要人工判断：换 URL、稍后重试，或接受现状

确定性的失败（404、403）不会重试，避免浪费时间。

### `fetch_urls(urls, max_chars, max_concurrent)`

批量抓，最多 20 条。**逐条独立成败**——一条失败不影响其他，返回里带 `ok_count`。

**批量要控制规模**。实测每条平均约 5 秒，单条最坏 8～10 秒（需要重试的骨架页）。按并发 3 算：
...
多数 MCP 客户端有 30 秒左右的调用超时。**建议一次不超过 8 条**，超过就分批；急用时把 `max_concurrent` 提到 5～8，但别超过 8（共享浏览器会吃不消）。
